...
```powershell
# Restore + build
dotnet build .\MdBrowser.sln

# Run
dotnet run --project .\src\MdBrowser\MdBrowser.csproj
```
...
The chosen flavor persists to `%LOCALAPPDATA%\MdBrowser\theme.txt`. Delete that
...
#     Prompts for a PFX password; writes certs/MdBrowser.pfx and MdBrowser.cer.
.\packaging\New-MdBrowserCert.ps1

# (3) Trust the public cert on this machine so MSIX install accepts it.
#     Needs an elevated PowerShell.
Import-Certificate -FilePath .\packaging\certs\MdBrowser.cer `
    -CertStoreLocation Cert:\LocalMachine\TrustedPeople
```

The cert's subject (`CN=MdBrowser Dev, O=MdBrowser, C=SE` by default) **must
...
# Publish (dotnet) -> pack (makeappx) -> sign (signtool). Output: packaging/out/MdBrowser.msix
.\packaging\Build-Msix.ps1 `
    -PfxPath .\packaging\certs\MdBrowser.pfx `
    -PfxPassword $pw

# Install it
Add-AppxPackage .\packaging\out\MdBrowser.msix
```
...
```powershell
Get-AppxPackage MdBrowser.MarkdownViewer | Remove-AppxPackage
```
...
- Mutex name: `Global\MdBrowser.Singleton.B1A47A1F-7C6E-4F2A-9D7B-7A2C8E1E3F11`
- Pipe name: `MdBrowser.Pipe.B1A47A1F-7C6E-4F2A-9D7B-7A2C8E1E3F11`
...
src/MdBrowser/
  ...
  Assets/
    MdBrowser.ico                Window icon (16/32/48/64/128/256)
  Themes/
...
packaging/
  ...
  New-MdBrowserCert.ps1          Generates code-signing PFX + public CER
...
```powershell
& 'C:\Program Files (x86)\Windows Kits\10\bin\10.0.26100.0\x64\signtool.exe' sign /fd SHA256 /f .\packaging\certs\MdBrowser.pfx /p <pw> .\packaging\out\MdBrowser.msix
```
